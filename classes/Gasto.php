...
class Gasto {
    ...
    public function create() {
        ...
        if (isset($_FILES["foto"]) && $_FILES["foto"]["error"] == UPLOAD_ERR_OK && !empty($_FILES["foto"]["tmp_name"])) {
            ...
            $new_file_name = $this->fecha . '_gasto_' . $clean_concepto . '_' . $unique_id . '.' . $imageFileType;
    ...
    public function read($usuario_id, $limit = null, $offset = null) {
        $query = "SELECT g.id, g.base_imponible, g.total_bruto, g.importe_iva, g.fecha, g.concepto, g.descripcion, g.foto_link,
                          cg.nombre as categoria_nombre, ti.porcentaje as tipo_iva_porcentaje
                  FROM " . $this->table_name . " g
                  LEFT JOIN categoria_gasto cg ON g.categoria_id = cg.id
                  LEFT JOIN " . $this->table_tipos_iva . " ti ON g.id_tipo_iva = ti.id
                  WHERE g.usuario_id = :usuario_id
                  ORDER BY g.fecha DESC, g.id DESC"; // Añadimos g.id DESC para un orden consistente si las fechas son iguales
    ...
    // Nuevo método para contar el total de gastos para un usuario
    ...
    public function readOne() {
        $query = "SELECT g.id, g.usuario_id, g.categoria_id, g.base_imponible, g.total_bruto, g.importe_iva, g.id_tipo_iva, g.fecha, g.concepto, g.descripcion, g.foto_link,
                          cg.nombre as categoria_nombre, ti.porcentaje as tipo_iva_porcentaje
                  FROM " . $this->table_name . " g
                  LEFT JOIN categoria_gasto cg ON g.categoria_id = cg.id
                  LEFT JOIN " . $this->table_tipos_iva . " ti ON g.id_tipo_iva = ti.id
                  WHERE g.id = ? AND g.usuario_id = ? LIMIT 0,1";
    ...
    public function update($remove_existing_foto = false) { 
        ...
        $old_gasto = new Gasto($this->conn); 
        $old_gasto->id = $this->id;
        $old_gasto->usuario_id = $this->usuario_id;
        $old_gasto->readOne();
        $old_foto_link = $old_gasto->foto_link; // Guarda el nombre de la foto actual de la DB
    ...
    public function delete() {
        ...
        $old_gasto->usuario_id = $this->usuario_id;
        $old_gasto->readOne();
        $old_foto_link = $old_gasto->foto_link;
    ...
    public function readByDateRange($usuario_id, $fecha_inicio, $fecha_fin) {
        $query = "SELECT
                    g.id, g.base_imponible, g.total_bruto, g.importe_iva, g.fecha, g.concepto, g.descripcion, g.foto_link,
                    cg.nombre as categoria_nombre, ti.porcentaje as tipo_iva_porcentaje
                  FROM
                    " . $this->table_name . " g
                  LEFT JOIN
                    categoria_gasto cg ON g.categoria_id = cg.id
                  LEFT JOIN
                    " . $this->table_tipos_iva . " ti ON g.id_tipo_iva = ti.id
                  WHERE
                    g.usuario_id = :usuario_id AND g.fecha BETWEEN :fecha_inicio AND :fecha_fin
                  ORDER BY
                    g.fecha DESC";
